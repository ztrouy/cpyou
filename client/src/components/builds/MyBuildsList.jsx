    ...
    return (
        <Container sx={{display: "flex", justifyContent: "center", mb: 5}}>
            <Box sx={{width: 1, maxWidth: "800px"}}>
                <Typography variant="h3" sx={{my: 2}}>My Builds</Typography>
                <Box sx={{display: "flex", flexDirection: "column", gap: 1}}>
                    {builds.length > 0 ? (
                        builds.map(b => (
                            <Paper sx={{p: 2}} elevation={3} key={`b-${b.id}`}>
                                <Box sx={{display: "flex", gap: 1}}>
                                    <ButtonBase onClick={() => navigate(`/builds/${b.id}`)}>
                            ...
                            <Typography textAlign={"center"} marginTop={5}>You have no builds</Typography>
                        )
                    }
                </Box>
            </Box>
